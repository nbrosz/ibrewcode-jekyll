...
<div class="container">
  ...
  <h2 id="email-header">Talk to me</h2>
  ...
  <div id="form" class="contact-form">
    <form accept-charset="UTF-8" method="POST" action="https://formspree.io/{{ site.email }}" v-on:submit.prevent="validateBeforeSubmit" ref="contact">
      <fieldset>
        <input type="hidden" name="_subject" value="New Contact Submission" id="email-title" />
...
var info = window.location.hash.substr(1).split("?");
if (info) {
  var titleElem = document.getElementById("email-title");
  var headerElem = document.getElementById("email-header");
  switch(info[0].toLowerCase()) {
    case "error": 
    var extra = info.length > 1 ? " - " + info[1] : "";
    titleElem.value += " - Error Report" + extra;
    headerElem.innerHTML = "Report Error";
    break;
  }
}
</script>
